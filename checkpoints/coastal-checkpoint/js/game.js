...
class Game {
  constructor(canvas) {
    ...
    // California Coast Rescue: ecosystem restoration HUD.
    this.ecoHealth = 0;
    this.ecoProgress = 0;
    this.ecoTotal = 1;
  ...
  applyHostState(s) {
    ...
    if (typeof s.ecoHealth === "number") this.ecoHealth = s.ecoHealth;
    if (s.animals && this.animals) {
      for (const na of s.animals) {
        const local = this.animals.find(a => a.type === na.type && Math.round(a.x) === na.x && Math.round(a.y) === na.y);
        if (local && na.rescued) { local.rescued = true; local.rescueTimer = 0; }
      }
    }
  ...
  addEcoProgress(amount = 1) {
    this.ecoProgress = Math.min(this.ecoTotal, (this.ecoProgress || 0) + amount);
    this.ecoHealth = Math.round((this.ecoProgress / Math.max(1, this.ecoTotal)) * 100);
  }

  rescueAnimal(animal, rescuer) {
    if (!animal || !animal.rescue()) return;
    this.questLog.onRescue(animal.type);
    this.addEcoProgress(1);
    Score.add(20);
    Particles.burst(animal.x + animal.width / 2, animal.y + animal.height / 2, "#9ad9b0", 22);
    Floaters.spawn(animal.x + animal.width / 2, animal.y - 6, animal.rescueText, "#9ad9b0");
    Sound.play("quest");
  }
  ...
  restoreQuests(saved) {
    ...
    if (saved._tallies) {
      ...
      this.questLog.rescues = saved._tallies.rescues || {};
    }
  }
  ...
  shopCatalog() {
    return [
      { label: "First-aid break (+10 energy)", price: 5, buy: (p) => p.heal(10) },
      { label: "Hydration pack (+5 max energy)", price: 15, buy: (p) => { p.maxHp += 5; p.hp += 5; } },
      { label: "Cleanup tool upgrade (+2 reach)", price: 20, buy: (p) => { p.attackDamage += 2; } },
    ];
  }
  openShop(npc) {
  ...
  drawWinScreen(ctx) {
    ...
    ctx.fillText("COVE RESTORED!", 0, 0);
    ctx.restore();
    ...
    ctx.fillText("You protected tidepools, shorebirds, sea turtles, and otters.", W / 2, H / 2 + 36);
    ctx.fillStyle = "#e6fff5";
    ctx.font = "14px monospace";
    ctx.fillText("The coastline is healthier because your team removed debris before it reached the ocean.", W / 2, H / 2 + 58);
    ctx.fillStyle = "#cfe8ff";
    ctx.font = "16px monospace";
    ctx.fillText("Press ENTER for the title screen", W / 2, H / 2 + 86);
    ctx.globalAlpha = 1;
    ctx.textAlign = "left";
  }
  ...
  drawScoreHud(ctx) {
    ...
    ctx.fillText("Restore Pts " + Score.value, rx, 118);
    ctx.fillStyle = "#cadcfc";
    ctx.fillText("Best " + Score.high(), rx, 134);
    if (Sound.muted) { ctx.fillStyle = "#ff8a8a"; ctx.fillText("MUTED (M)", rx, 150); }
    ctx.textAlign = "left";
  }

  drawEcoHud(ctx) {
    if (!this.mapData || !this.mapData.ecosystem) return;
    const w = 220, h = 18;
    const x = ctx.canvas.width - w - 14;
    const y = ctx.canvas.height - 44;
    const pct = Math.max(0, Math.min(100, this.ecoHealth || 0));
    ctx.fillStyle = "rgba(22, 45, 54, 0.82)";
    ctx.fillRect(x - 6, y - 20, w + 12, 46);
    ctx.strokeStyle = "#7fd1ff";
    ctx.strokeRect(x - 5.5, y - 19.5, w + 11, 45);
    ctx.fillStyle = "#e6fff5";
    ctx.font = "bold 12px monospace";
    ctx.textAlign = "left";
    ctx.fillText("ECOSYSTEM HEALTH", x, y - 5);
    ctx.fillStyle = "#365061";
    ctx.fillRect(x, y, w, h);
    const grad = ctx.createLinearGradient(x, y, x + w, y);
    grad.addColorStop(0, "#65c7d9");
    grad.addColorStop(1, "#9ad9b0");
    ctx.fillStyle = grad;
    ctx.fillRect(x, y, w * (pct / 100), h);
    ctx.fillStyle = "#ffffff";
    ctx.font = "bold 12px monospace";
    ctx.textAlign = "right";
    ctx.fillText(`${Math.round(pct)}%`, x + w - 6, y + 13);
    ctx.textAlign = "left";
  }
  ...
  checkProgress() {
    ...
    if (allDone) {
      // California Coast Rescue is a one-map restoration project: once the
      // cleanup + rescue mission is complete, the habitat is restored.
      Sound.play("quest");
      this.startWin();
      return;
    }
  ...
  loadWorld(carry) {
    ...
    for (const e of this.mapData.entities) {
      ...
      else if (e.kind === "animal") this.animals.push(new Animal(e));
      else if (e.kind === "enemy") this.enemies.push(new Enemy(e));
      else if (e.kind === "item") this.items.push(new Item(e));
    }
    // Ecosystem health is a clean-progress meter: every pollution item cleaned
    // and every animal rescued fills one slice of the bar.
    this.ecoProgress = 0;
    const requiredEcoSteps = this.mapData.ecosystem && this.mapData.ecosystem.totalSteps;
    this.ecoTotal = Math.max(1, requiredEcoSteps || (this.items.filter(i => i.eco).length + this.animals.length));
    this.ecoHealth = 0;
  ...
  broadcastState() {
    const snap = {
      ...
      animals: (this.animals || []).map(a => ({ type: a.type, x: Math.round(a.x), y: Math.round(a.y), rescued: !!a.rescued })),
      ecoHealth: this.ecoHealth,
  ...
  updatePlaying(dt) {
    ...
    this.nearbyAnimal = (this.animals || []).find(a => !a.rescued && a.isNear(this.player));
    if (this.nearbyAnimal && Input.wasPressed("KeyE")) {
      this.rescueAnimal(this.nearbyAnimal, this.player);
    }
    ...
    // ----- Wildlife rescue: get close or press E near an animal -----
    for (const animal of (this.animals || [])) {
      animal.update(dt);
      if (!animal.rescued) {
        const rescuer = this.players.find(p => animal.overlaps(p));
        if (rescuer) this.rescueAnimal(animal, rescuer);
      }
    }
    this.animals = (this.animals || []).filter(a => !a.doneFade);
    ...
    for (const item of this.items) {
      ...
      if (picker) {
        ...
        Score.add(item.scoreValue ?? 5);   // restoration points on cleanup
        if (item.eco) this.addEcoProgress(1);
  ...
  draw() {
    ...
    if (this.state === STATE.LOADING) {
      UI.drawScreen(ctx, "Loading...", "Loading tide charts and rescue gear");
      return;
    }
    if (this.state === STATE.TITLE) {
      const sub = this.hasSave() ? "SPACE - new game    L - continue" : "Press SPACE to begin";
      UI.drawScreen(ctx, "California Coast Rescue", sub, "#7fd1ff");
      // Online multiplayer hints + the host's room code once it exists.
      ctx.textAlign = "center";
      ctx.font = "13px monospace";
      ctx.fillStyle = "#cadcfc";
      ctx.fillText("MISSION: clean pollution, rescue wildlife, restore the cove", ctx.canvas.width / 2, ctx.canvas.height - 70);
      ctx.fillText("ONLINE:  H - host a game     J - join a game", ctx.canvas.width / 2, ctx.canvas.height - 50);
    ...
    const visibleAnimals = (this.animals || []).filter(a => !a.doneFade);
    const things = [...this.items, ...visibleAnimals, ...this.npcs, ...visibleEnemies, ...this.players];
    ...
    this.drawEcoHud(ctx);                        // ecosystem health meter
    UI.drawQuests(ctx, this.questLog, minimapShowing); // draw last so missions never get hidden
    ...
    if (this.state === STATE.PLAYING && this.nearbyAnimal) {
      UI.drawPrompt(ctx, `Press E or walk close to rescue ${this.nearbyAnimal.name}`);
    } else if (this.state === STATE.PLAYING && this.nearbyNpc) {
