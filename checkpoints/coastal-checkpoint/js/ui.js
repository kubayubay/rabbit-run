...
export const UI = {
  ...
  drawHealth(ctx, player) {
    ...
    ctx.textAlign = "left";
    ctx.fillText(`ENERGY ${player.hp}/${player.maxHp}`, x + 6, y + 13);
    ...
    ctx.fillText(`TEAM ${player.level}`, x + 6, xpY + 8);
    ctx.textAlign = "right";
    ctx.fillText(`TRAIN ${player.xp}/${player.xpToNext}`, x + w - 6, xpY + 8);
    ctx.textAlign = "left";
  ...
  drawQuests(ctx, questLog, minimapShowing) {
    ...
    if (!questLog.hudVisible) {
      ...
      ctx.fillText("Missions hidden (Q)", hintX, 70);
      ctx.textAlign = "left";
      return;
    }
    ...
    ctx.fillText("MISSIONS  (Q to hide)", x + 10, ty);
    ty += 18;
  ...
  drawInventory(ctx, inventory) {
    ...
    ctx.textAlign = "center";
    ctx.fillText("CLEANUP LOG", CONFIG.CANVAS_WIDTH/2, y + 30);
