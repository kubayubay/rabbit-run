...
const FRAMES = { idle: 8, run: 8, sword: 8 };
...
export class Player {
  constructor(x, y, keys = KEYMAP_P1) {
    ...
    // Where this player reads its keys from. Normally the real keyboard (Input).
    // For an ONLINE remote bunny we swap in a NetInputSource that has the same
    // isDown()/wasPressed() shape but is fed by network messages (see net.js).
    // This one line is what lets the SAME Player code be driven by a faraway
    // friend's keyboard with no other changes.
    this.input = Input;
  ...
  // Start a timed boost, or STACK onto one that's already running. Grabbing another
  // carrot while a speed boost is active adds its seconds on top of what's left
  // (e.g. 5s left + a new 5s carrot = 10s), rather than just resetting to 5s. We
  // grow `total` too so the countdown bar still fills to the right proportion.
  addBoost(id, seconds, label, color) {
    const existing = this.boosts[id];
    if (existing && existing.remaining > 0) {
      existing.remaining += seconds;
      existing.total += seconds;
      // keep the latest label/color in case they ever differ
  ...
  update(dt, map) {
    ...
    if (this.input.wasPressed(this.keys.dash) && this.dashCooldown <= 0) {
    ...
    if (this.input.wasPressed(this.keys.attack)) {
      this.startAttack();
      return;
    }
    ...
    let dx = 0, dy = 0;
    if (this.input.isDown(this.keys.left))  { dx -= 1; this.dir = DIR.LEFT; }
    if (this.input.isDown(this.keys.right)) { dx += 1; this.dir = DIR.RIGHT; }
    if (this.input.isDown(this.keys.up))    { dy -= 1; this.dir = DIR.UP; }
    if (this.input.isDown(this.keys.down))  { dy += 1; this.dir = DIR.DOWN; }
  ...
  draw(ctx, camera) {
    ...
    // On the guest, a remote bunny's dash and boost arrive as the _netDash / _netBoost
    // flags (the real timers live on the host), so we check those too.
    const boosting = this.hasBoost("speed") || this._netBoost;
    const dashing = this.dashTimer > 0 || this._netDash;
    if ((dashing || boosting) && this.trail.length > 0) {
    ...
    // On the guest, the remote bunny's i-frames live in _netHurt (synced from the host).
    const hurtTimer = Math.max(this.invincibleTimer, this._netHurt || 0);
    const hurtBlink = hurtTimer > 0 && this.dashTimer <= 0;
    if (hurtBlink && Math.floor(hurtTimer * 12) % 2 === 0) {
      return;
    }
  ...
  // The host describes a bunny in a tiny object; the guest copies those values
  // onto its own copy so it can draw the bunny in the right place/pose. We only
  // send what's needed to DRAW and to show health - not the whole object.
  netSnapshot() {
    return {
      x: Math.round(this.x), y: Math.round(this.y),
      dir: this.dir, moving: this.moving, attacking: this.attacking,
      hp: Math.round(this.hp), maxHp: this.maxHp,
      frame: this.anim.frameIndex,       // animation frame hint (guest animates locally; see netAnimate)
      boost: this.hasBoost("speed"),     // so the guest can draw the speed streak
      dashing: this.dashTimer > 0,       // so the guest can draw the dash afterimage too
      // The afterimage trail (recent positions) lives only on the host, so send it
      // along - it's a short list of {x,y,dir} and lets the guest draw the streak.
      trail: this.trail.map(t => ({ x: Math.round(t.x), y: Math.round(t.y), dir: t.dir })),
      hurt: this.invincibleTimer > 0 && this.dashTimer <= 0,  // so the guest sees the hit-blink
      gold: this.gold || 0,              // each bunny's own coins, so the guest can shop
      level: this.level, xp: this.xp, xpToNext: this.xpToNext,  // so the guest's HP/XP HUD is its own
      // active timed power-ups, so the guest can draw its OWN countdown bar
      boosts: Object.keys(this.boosts).map(id => ({
        id, remaining: this.boosts[id].remaining, total: this.boosts[id].total,
        label: this.boosts[id].label, color: this.boosts[id].color,
      })),
    };
  }
  applyNetSnapshot(s) {
    // INTERPOLATION: instead of snapping straight to the host's position, we store
    // it as a TARGET and glide toward it a little each frame (see netLerp below).
    // Snapshots only arrive ~30x/sec, so without this the remote bunny would jump;
    // gliding makes it look smooth at 60fps. The very first snapshot snaps into
    // place so the bunny doesn't slide in from (0,0).
    if (this._netInit) {
      this.targetX = s.x; this.targetY = s.y;
    } else {
      this.x = this.targetX = s.x; this.y = this.targetY = s.y;
      this._netInit = true;
    }
    // Detect the start of a swing (attacking goes false -> true) so the guest
    // hears the sword sound for BOTH bunnies, even though it doesn't simulate them.
    if (s.attacking && !this.attacking) Sound.play("attack");
    this.dir = s.dir; this.moving = s.moving; this.attacking = s.attacking;
    this.hp = s.hp; this.maxHp = s.maxHp;
    this.displayedHp = s.hp;
    if (typeof s.gold === "number") this.gold = s.gold;   // keep each bunny's coins in sync
    if (typeof s.level === "number") this.level = s.level;
    if (typeof s.xp === "number") this.xp = s.xp;
    if (typeof s.xpToNext === "number") this.xpToNext = s.xpToNext;
    // Rebuild the active power-up list so the guest can draw its own countdown bar.
    // (The host owns the real boost objects; we just mirror enough to display.)
    if (s.boosts) {
      this.boosts = {};
      for (const bo of s.boosts) {
        this.boosts[bo.id] = { remaining: bo.remaining, total: bo.total, label: bo.label, color: bo.color };
      }
    }
    this._netBoost = s.boost;            // remembered just for the trail visual
    this._netDash = s.dashing;           // remembered so the guest draws the dash streak
    if (s.trail) this.trail = s.trail;   // copy the host's afterimage trail to draw it here
    // If the host says this bunny just got hit, run a short local blink timer so
    // the guest sees the same flash. We count it down in netLerp.
    if (s.hurt) this._netHurt = 0.5;
    // NOTE: we do NOT copy s.frame onto the animator here. Snapshots arrive only
    // ~30x/sec, so copying a single frame would make the legs look frozen/choppy.
    // Instead the guest runs the walk cycle LOCALLY at 60fps in netAnimate(),
    // driven by the moving/attacking flags above. That's what makes the remote
    // bunny look like it's walking perfectly smoothly.
  }

  // Glide toward the latest network target AND advance the animation locally.
  // Called every frame on the guest for each bunny it's drawing. The position
  // lerp (0.35 of the gap per frame) smooths the ~30/sec snapshots into fluid
  // motion; running the animator here keeps the legs cycling at a full 60fps.
  // lerp smooths the ~30/sec snapshots into fluid 60fps motion. Two tweaks keep it
  // from feeling laggy: (1) if we've fallen a long way behind (a delayed snapshot,
  // or a teleport), SNAP most of the way instead of crawling there slowly; (2) glide
  // a bit more aggressively (0.35) so the bunny stays close to the latest snapshot.
  netLerp(dt = 1 / 60) {
    if (this.targetX === undefined) return;
    const dx = this.targetX - this.x, dy = this.targetY - this.y;
    const dist = Math.hypot(dx, dy);
    const SNAP_DIST = 140;     // ~3 tiles: beyond this we're clearly behind, so catch up hard
    if (dist > SNAP_DIST) {
      // jump 80% of the way immediately, then let the glide finish the rest
      this.x += dx * 0.8;
      this.y += dy * 0.8;
    } else {
      this.x += dx * 0.35;     // normal smooth glide toward the latest target
      this.y += dy * 0.35;
    }
    this.netAnimate(dt);
    if (this._netHurt > 0) this._netHurt -= dt;   // fade the hit-blink
  }

  // Pick the right animation (attack > walk > idle) from the networked flags and
  // advance it locally, exactly like Player.update does for a real keyboard bunny.
  netAnimate(dt) {
    if (this.attacking)   this.anim.update(dt, FRAMES.sword);
    else if (this.moving) this.anim.update(dt, FRAMES.run);
    else                  this.anim.update(dt, FRAMES.idle);
  }
}
