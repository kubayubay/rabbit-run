...
export class Item {
  constructor(data) {
    ...
    // California Coast Rescue: items can improve the ecosystem meter when cleaned up.
    this.eco = !!data.eco;
    this.scoreValue = data.scoreValue ?? 5;
  ...
  draw(ctx, camera) {
    if (this.collected) return;            // picked up - don't draw it anymore
