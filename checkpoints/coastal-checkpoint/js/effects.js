...
export const Score = {
  ...
  add(points) {
    this.value += points;
    const best = this.high();
    if (this.value > best) {
      this._bestMemory = this.value;
      try { localStorage.setItem("highScore", this.value); } catch (e) {}
    }
  },
  high() {
    try { return Number(localStorage.getItem("highScore") || this._bestMemory || 0); }
    catch (e) { return this._bestMemory || 0; }
  },
};
