...
export class Quest {
  ...
  creditFromHistory(history) {
    ...
    const rescues = history.rescues || {};   // { animalType: countRescued }
    for (const o of this.objectives) {
      ...
      else if (o.type === "rescue")     have = rescues[o.target] || 0;
      else continue;
...
export class QuestLog {
  constructor() {
    ...
    this.rescues = {};       // animalType -> how many have been rescued
  }
  ...
  start(id, inventory) {
    ...
    if (q && !q.started) {
      ...
      q.creditFromHistory({ inventory, defeats: this.defeats, visited: this.visited, talked: this.talked, rescues: this.rescues });
      return q;
    }
    return null;
  }
  ...
  onRescue(animalType) {
    this.rescues[animalType] = (this.rescues[animalType] || 0) + 1;
    this.progress("rescue", animalType);
    this.progress("rescue", "animal");
  }
  onReach(locationId) {
