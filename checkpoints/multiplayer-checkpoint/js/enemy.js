...
// A simple counter that gives every enemy a unique, stable network id. The guest
// matches enemies to the host's snapshot by this id, so the right sprite always
// follows the right enemy even as enemies die and new waves spawn (matching by
// array index breaks the moment the lists fall out of step).
let NEXT_ENEMY_ID = 1;
...
export class Enemy {
  constructor(data) {
    this.eid = NEXT_ENEMY_ID++;   // stable id for network sync
